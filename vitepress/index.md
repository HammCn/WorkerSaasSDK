...
> [!Important]
> 欢迎程序员兄弟访问理账狮开放平台接口文档，如有疑问或者在对接过程中产生疑问，请与我们技术支持人员联系。
...
- 正式环境：`https://open.workersaas.com/api/open/`
- 沙箱环境：`https://sandbox.workersaas.com/api/open/`


> [!TIP]
> 请使用上面的 网关 + API 地址进行组合最后的请求地址。
...
sequenceDiagram
    ...
    园区 --) 理账狮 : 园区审核任务
    用户 -->> 理账狮 : 签约并接受任务
    发包企业 ->> 理账狮 : 调用API获取可用任务列表
    理账狮 -) 发包企业 : 返回可用任务列表
    发包企业 ->> 理账狮 : 调用API创建支付订单
    理账狮 -) 发包企业 : 订单创建成功
    理账狮-->> 发包企业 : 异步通知订单信息
    发包企业 -->> 理账狮 : 回复 `success`
